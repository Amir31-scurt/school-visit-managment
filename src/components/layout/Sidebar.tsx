    ...
    <aside className="w-[var(--sidebar-width)] bg-[rgb(var(--color-primary))] text-white h-screen flex flex-col shrink-0 sticky top-0">
      <div className="p-6 border-b border-white/10 flex items-center justify-center">
        <img
          src={logo}
          alt="Logo"
          className="h-12 object-contain bg-white/10 p-2 rounded-xl backdrop-blur-sm"
        />
      </div>
      {/* show pending badge on Requests nav item */}
      <nav className="flex-1 py-6 flex flex-col gap-2 px-4">
        {(() => {
          const {requests: pending} = useRequests({status: "pending"});
